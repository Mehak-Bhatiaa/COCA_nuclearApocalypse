...
namespace LandscapeBuilder
{
    ...
    public class LBTerrainGrass
    {
        ...
        // Added 2.4.0 Beta 4b - used with URP & HDRP 2021.2+
        public bool useInstancing;

        // Added v2.3.3
        public bool useImportedDensity;
        ...
        public LBTerrainGrass(LBTerrainGrass lbTerrainGrass)
        {
            ...
            this.useInstancing = lbTerrainGrass.useInstancing;
            this.useImportedDensity = lbTerrainGrass.useImportedDensity;
        ...
        /// <summary>
        /// Does grass match the search criteria.
        ...
        {
            bool isMatch = false;
            string grassNameText = string.Empty;
            ...
            else
            {
                if (useMeshPrefab)
                {
                    if (meshPrefab != null) { grassNameText = meshPrefab.name; }
                    else if (!string.IsNullOrEmpty(meshPrefabName)) { grassNameText = meshPrefabName + " N/A"; }
                    else { grassNameText = "No grass prefab"; }

                    isMatch = (!string.IsNullOrEmpty(grassNameText) && grassNameText.ToLower().Contains(searchFilter.ToLower()));
                }
                else
                {                   
                    // If this grass has no textureName, it can't be a match
                    isMatch = !string.IsNullOrEmpty(textureName) && textureName.ToLower().Contains(searchFilter.ToLower());

                    // The textureName string may not have been updated, so check actual texture name
                    if (!isMatch && texture != null)
                    {
        ...
        public string ScriptGrass(int GrassIdx, string EndOfLineMarker = "\n")
        {
            ...
            sb.Append("\t" + grInst + ".useInstancing = " + useInstancing.ToString().ToLower() + "; " + eol);
            #endif
            sb.Append("\t" + grInst + ".useImportedDensity = " + useImportedDensity.ToString().ToLower() + "; " + eol);
        ...
        public static List<DetailPrototype> ToDetailPrototypeList(List<LBTerrainGrass> terrainGrassList)
        {
            if (terrainGrassList != null)
            {
                ...
                for (int i = 0; i < terrainGrassList.Count; i++)
                {
                    ...
                    #if UNITY_2021_2_OR_NEWER
                    temp.useInstancing = terrainGrassList[i].useInstancing;
                    #endif
            ...
            if (detailPrototypes != null)
            {
                if (detailPrototypes.Length > 0)
                {
                    ...
                    for (int i = 0; i < detailPrototypes.Length; i++)
                    {
                        ...
                        #if UNITY_2021_2_OR_NEWER
                        temp.useInstancing = detailPrototypes[i].useInstancing;
                        #endif
